  ...
  return (
    <div>
      ...
      <Grid container spacing={8}>
        {videos.map((video) => (
          <Grid key={video.id} item xs={12} sm={12} md={12} lg={6} xl={6} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Link href={`/video/${video.id}`}>
              <div style={{ position: 'relative', width: 640, height: 360 }}>
                ...
                <Image src={video.thumbnail} width="640" height="360" alt={video.title} style={{ opacity: 0.5 }} />
              </div>
            </Link>
          </Grid>
        ))}
